...
class IsingModelHoneycomb():
    def __init__(self, J, T, row, col, spin_config=None):
        ...
        """
        number of sites on a row are different.
        make the convention that:
            long rows have col number of sites, 
            and short rows have (col-1) IsingModelHoneycombumber of sites.
        then, half of the rows are long and the other half are short
        """
...
for t in np.arange(.05,4,.05):
    temp = IsingModelHoneycomb(1,t,8,4,0)
